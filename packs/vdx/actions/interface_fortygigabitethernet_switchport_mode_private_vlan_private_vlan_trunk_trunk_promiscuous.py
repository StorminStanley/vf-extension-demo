...
def interface_fortygigabitethernet_switchport_mode_private_vlan_private_vlan_trunk_trunk_promiscuous(**kwargs):
    """Auto Generated Code
    """
    ...
    if kwargs.pop('delete_trunk_promiscuous', False) is True:
        delete_trunk_promiscuous = config.find('.//*trunk-promiscuous')
        delete_trunk_promiscuous.set('operation', 'delete')
        

    callback = kwargs.pop('callback', _callback)
    return callback(config, mgr=kwargs.pop('mgr'))

class interface_fortygigabitethernet_switchport_mode_private_vlan_private_vlan_trunk_trunk_promiscuous_act(Action):
    def run(self, delete_switchport, name, delete_fortygigabitethernet, delete_private_vlan, delete_interface, delete_name, delete_trunk_promiscuous, delete_mode, delete_private_vlan_trunk, host, username, password):
        ...
        mgr.agtimeout = 600
        interface_fortygigabitethernet_switchport_mode_private_vlan_private_vlan_trunk_trunk_promiscuous(delete_trunk_promiscuous=delete_trunk_promiscuous, name=name, delete_fortygigabitethernet=delete_fortygigabitethernet, delete_switchport=delete_switchport, username=username, delete_mode=delete_mode, host=host, delete_private_vlan=delete_private_vlan, delete_name=delete_name, delete_interface=delete_interface, delete_private_vlan_trunk=delete_private_vlan_trunk, password=password, callback=_callback, mgr=mgr)
        return 0
