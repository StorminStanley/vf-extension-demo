...
def interface_fortygigabitethernet_ipv6_ipv6_nd_ra_ipv6_intf_cmds_nd_prefix_lifetime_preferred_ch_preferred_type_ca_preferred_lifetime_preferred_lifetime(**kwargs):
    """Auto Generated Code
    """
    ...
    if kwargs.pop('delete_ca_preferred_lifetime', False) is True:
        delete_ca_preferred_lifetime = config.find('.//*ca-preferred-lifetime')
        delete_ca_preferred_lifetime.set('operation', 'delete')
        
    preferred_lifetime = ET.SubElement(ca_preferred_lifetime, "preferred-lifetime")
    if kwargs.pop('delete_preferred_lifetime', False) is True:
        delete_preferred_lifetime = config.find('.//*preferred-lifetime')
        delete_preferred_lifetime.set('operation', 'delete')
...
class interface_fortygigabitethernet_ipv6_ipv6_nd_ra_ipv6_intf_cmds_nd_prefix_lifetime_preferred_ch_preferred_type_ca_preferred_lifetime_preferred_lifetime_act(Action):
    def run(self, delete_ca_preferred_lifetime, delete_preferred, delete_prefix, delete_fortygigabitethernet, preferred_lifetime, delete_ch_preferred_type, delete_interface, delete_name, delete_lifetime, delete_nd, delete_preferred_lifetime, name, host, username, password):
        ...
        mgr.agtimeout = 600
        interface_fortygigabitethernet_ipv6_ipv6_nd_ra_ipv6_intf_cmds_nd_prefix_lifetime_preferred_ch_preferred_type_ca_preferred_lifetime_preferred_lifetime(name=name, delete_prefix=delete_prefix, delete_lifetime=delete_lifetime, delete_name=delete_name, delete_ca_preferred_lifetime=delete_ca_preferred_lifetime, delete_nd=delete_nd, delete_preferred_lifetime=delete_preferred_lifetime, host=host, delete_preferred=delete_preferred, delete_ch_preferred_type=delete_ch_preferred_type, delete_fortygigabitethernet=delete_fortygigabitethernet, preferred_lifetime=preferred_lifetime, username=username, delete_interface=delete_interface, password=password, callback=_callback, mgr=mgr)
        return 0
