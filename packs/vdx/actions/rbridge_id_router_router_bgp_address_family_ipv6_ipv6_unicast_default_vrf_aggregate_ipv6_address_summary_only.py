...
def rbridge_id_router_router_bgp_address_family_ipv6_ipv6_unicast_default_vrf_aggregate_ipv6_address_summary_only(**kwargs):
    """Auto Generated Code
    """
    ...
    if kwargs.pop('delete_summary_only', False) is True:
        delete_summary_only = config.find('.//*summary-only')
        delete_summary_only.set('operation', 'delete')
        

    callback = kwargs.pop('callback', _callback)
    return callback(config, mgr=kwargs.pop('mgr'))

class rbridge_id_router_router_bgp_address_family_ipv6_ipv6_unicast_default_vrf_aggregate_ipv6_address_summary_only_act(Action):
    def run(self, delete_address_family, delete_router_bgp, rbridge_id, delete_default_vrf, delete_aggregate_ip_prefix, delete_rbridge_id, delete_summary_only, delete_router, aggregate_ip_prefix, host, username, password):
        ...
        mgr.agtimeout = 600
        rbridge_id_router_router_bgp_address_family_ipv6_ipv6_unicast_default_vrf_aggregate_ipv6_address_summary_only(aggregate_ip_prefix=aggregate_ip_prefix, delete_router_bgp=delete_router_bgp, username=username, delete_rbridge_id=delete_rbridge_id, rbridge_id=rbridge_id, delete_router=delete_router, delete_summary_only=delete_summary_only, host=host, delete_aggregate_ip_prefix=delete_aggregate_ip_prefix, delete_address_family=delete_address_family, delete_default_vrf=delete_default_vrf, password=password, callback=_callback, mgr=mgr)
        return 0
