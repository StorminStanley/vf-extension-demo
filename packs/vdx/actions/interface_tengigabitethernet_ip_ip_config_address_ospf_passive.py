...
def interface_tengigabitethernet_ip_ip_config_address_ospf_passive(**kwargs):
    """Auto Generated Code
    """
    ...
    if kwargs.pop('delete_ospf_passive', False) is True:
        delete_ospf_passive = config.find('.//*ospf-passive')
        delete_ospf_passive.set('operation', 'delete')
        

    callback = kwargs.pop('callback', _callback)
    return callback(config, mgr=kwargs.pop('mgr'))

class interface_tengigabitethernet_ip_ip_config_address_ospf_passive_act(Action):
    def run(self, delete_ip_config, name, delete_ip, delete_interface, delete_name, address, delete_tengigabitethernet, delete_address, delete_ospf_passive, host, username, password):
        ...
        mgr.agtimeout = 600
        interface_tengigabitethernet_ip_ip_config_address_ospf_passive(delete_ospf_passive=delete_ospf_passive, name=name, delete_name=delete_name, address=address, delete_ip_config=delete_ip_config, delete_ip=delete_ip, delete_tengigabitethernet=delete_tengigabitethernet, host=host, delete_address=delete_address, delete_interface=delete_interface, username=username, password=password, callback=_callback, mgr=mgr)
        return 0
