...
def rbridge_id_router_ospf_area_virtual_link_md5_authentication_key_activation_wait_time(**kwargs):
    """Auto Generated Code
    """
    ...
    if kwargs.pop('delete_key_activation_wait_time', False) is True:
        delete_key_activation_wait_time = config.find('.//*key-activation-wait-time')
        delete_key_activation_wait_time.set('operation', 'delete')
...
class rbridge_id_router_ospf_area_virtual_link_md5_authentication_key_activation_wait_time_act(Action):
    def run(self, area_id, delete_virtual_link, key_activation_wait_time, rbridge_id, delete_ospf, delete_vrf, delete_area_id, delete_key_activation_wait_time, delete_area, vrf, delete_virt_link_neighbor, delete_rbridge_id, virt_link_neighbor, delete_router, host, username, password):
        ...
        mgr.agtimeout = 600
        rbridge_id_router_ospf_area_virtual_link_md5_authentication_key_activation_wait_time(key_activation_wait_time=key_activation_wait_time, delete_virt_link_neighbor=delete_virt_link_neighbor, username=username, virt_link_neighbor=virt_link_neighbor, delete_area=delete_area, area_id=area_id, rbridge_id=rbridge_id, delete_vrf=delete_vrf, delete_key_activation_wait_time=delete_key_activation_wait_time, delete_router=delete_router, delete_ospf=delete_ospf, password=password, vrf=vrf, host=host, delete_rbridge_id=delete_rbridge_id, delete_virtual_link=delete_virtual_link, delete_area_id=delete_area_id, callback=_callback, mgr=mgr)
        return 0
