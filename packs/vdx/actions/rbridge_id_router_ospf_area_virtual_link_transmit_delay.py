...
def rbridge_id_router_ospf_area_virtual_link_transmit_delay(**kwargs):
    """Auto Generated Code
    """
...
class rbridge_id_router_ospf_area_virtual_link_transmit_delay_act(Action):
    def run(self, transmit_delay, delete_transmit_delay, area_id, delete_virtual_link, rbridge_id, delete_ospf, delete_vrf, delete_area_id, delete_area, vrf, delete_virt_link_neighbor, delete_rbridge_id, virt_link_neighbor, delete_router, host, username, password):
        ...
        mgr.agtimeout = 600
        rbridge_id_router_ospf_area_virtual_link_transmit_delay(delete_virt_link_neighbor=delete_virt_link_neighbor, username=username, virt_link_neighbor=virt_link_neighbor, delete_area=delete_area, delete_virtual_link=delete_virtual_link, rbridge_id=rbridge_id, delete_vrf=delete_vrf, password=password, vrf=vrf, delete_router=delete_router, delete_transmit_delay=delete_transmit_delay, host=host, transmit_delay=transmit_delay, delete_ospf=delete_ospf, delete_rbridge_id=delete_rbridge_id, area_id=area_id, delete_area_id=delete_area_id, callback=_callback, mgr=mgr)
        return 0
