...
def rbridge_id_interface_ve_vrrp_use_v2_checksum(**kwargs):
    """Auto Generated Code
    """
    ...
    if kwargs.pop('delete_use_v2_checksum', False) is True:
        delete_use_v2_checksum = config.find('.//*use-v2-checksum')
        delete_use_v2_checksum.set('operation', 'delete')
        

    callback = kwargs.pop('callback', _callback)
    return callback(config, mgr=kwargs.pop('mgr'))

class rbridge_id_interface_ve_vrrp_use_v2_checksum_act(Action):
    def run(self, name, delete_vrid, vrid, delete_version, delete_ve, version, delete_interface, delete_name, delete_vrrp, delete_rbridge_id, rbridge_id, host, username, password):
        ...
        mgr.agtimeout = 600
        rbridge_id_interface_ve_vrrp_use_v2_checksum(delete_vrid=delete_vrid, delete_vrrp=delete_vrrp, name=name, delete_version=delete_version, username=username, rbridge_id=rbridge_id, version=version, delete_ve=delete_ve, host=host, vrid=vrid, delete_name=delete_name, delete_interface=delete_interface, delete_rbridge_id=delete_rbridge_id, password=password, callback=_callback, mgr=mgr)
        return 0
