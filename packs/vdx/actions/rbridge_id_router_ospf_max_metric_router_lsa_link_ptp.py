...
def rbridge_id_router_ospf_max_metric_router_lsa_link_ptp(**kwargs):
    """Auto Generated Code
    """
    ...
    if kwargs.pop('delete_ptp', False) is True:
        delete_ptp = config.find('.//*ptp')
        delete_ptp.set('operation', 'delete')
        

    callback = kwargs.pop('callback', _callback)
    return callback(config, mgr=kwargs.pop('mgr'))

class rbridge_id_router_ospf_max_metric_router_lsa_link_ptp_act(Action):
    def run(self, rbridge_id, delete_ospf, delete_vrf, delete_ptp, delete_router_lsa, delete_link, delete_max_metric, vrf, delete_rbridge_id, delete_router, host, username, password):
        ...
        mgr.agtimeout = 600
        rbridge_id_router_ospf_max_metric_router_lsa_link_ptp(delete_max_metric=delete_max_metric, delete_router_lsa=delete_router_lsa, username=username, delete_rbridge_id=delete_rbridge_id, rbridge_id=rbridge_id, delete_vrf=delete_vrf, delete_ptp=delete_ptp, delete_router=delete_router, delete_ospf=delete_ospf, vrf=vrf, host=host, delete_link=delete_link, password=password, callback=_callback, mgr=mgr)
        return 0
